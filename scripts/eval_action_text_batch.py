#!/usr/bin/env python3
"""
Batch evaluation script for VLN Action Text model.
Evaluates at fixed intervals, processing all history frames each time.
"""
...
class BatchActionEvaluator:
    """Evaluator that processes history frames in batch (like training)."""
    ...
    def predict(self, images: List[np.ndarray], instruction: str) -> str:
        """Predict actions given a batch of history frames."""
        # Reset cache for fresh inference
        self.reset()
...
def evaluate_session(
    evaluator: BatchActionEvaluator,
    session_path: str,
    stride: int = 60,
    max_frames: int = 16,
    verbose: bool = False
) -> Dict:
    """Evaluate a session at fixed intervals."""
    # Load data
    ...
    # Evaluate at stride intervals
    for frame_idx in range(4, num_frames, stride):
        # Load history frames
        start_idx = max(0, frame_idx - max_frames + 1)
        images = []
        for i in range(start_idx, frame_idx + 1):
            img_file = trajectory[i]['image_file']
        ...
        if verbose:
            print(f"  Frame {frame_idx} ({len(images)} frames): "
                  f"GT={' '.join(gt_actions)}, Pred={' '.join(pred_actions)}, "
                  f"Correct={correct}/4")
...
def main():
    parser = argparse.ArgumentParser(description='Batch evaluation of VLN Action Text model')
    ...
    parser.add_argument('--output', type=str, default='eval_batch_results.json')
    parser.add_argument('--verbose', action='store_true')
    args = parser.parse_args()

    # Initialize evaluator
    evaluator = BatchActionEvaluator(args.model_path, args.lora_path)
    ...
    print(f"Stride: {args.stride}, Max frames: {args.max_frames}")
    print()
    ...
    for session_path in tqdm(session_dirs, desc="Sessions"):
        ...
        result = evaluate_session(
            evaluator, session_path, args.stride, args.max_frames, args.verbose
        )
    ...
    # Calculate exact match and first action accuracy
    total_samples = sum(r['num_samples'] for r in all_results)
    exact_matches = sum(
        1 for r in all_results for s in r['results']
        if s['pred_actions'] == s['gt_actions']
    )
    first_correct = sum(
        1 for r in all_results for s in r['results']
        if s['pred_actions'][0] == s['gt_actions'][0]
    )
    ...
    print(f"Exact match rate: {exact_matches/total_samples:.2%} ({exact_matches}/{total_samples})")
    print(f"First action accuracy: {first_correct/total_samples:.2%} ({first_correct}/{total_samples})")
    ...
    for r in all_results:
        print(f"  {r['session']}: {r['accuracy']:.2%} ({r['total_correct']}/{r['total_actions']})")
    ...
    output_data = {
        ...
        'metrics': {
            ...
            'first_action_accuracy': first_correct / total_samples,
        },
