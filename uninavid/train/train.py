...
class ModelArguments:
    ...
    use_waypoint_head: bool = field(default=False)
    num_waypoints: int = field(default=5)
    waypoint_loss_weight: float = field(default=1.0)
    angle_loss_weight: float = field(default=0.5)
    arrive_loss_weight: float = field(default=0.5)
    use_lm_loss_for_waypoint: bool = field(default=False)
...
def train():
    ...
    if data_args.use_omninav_waypoint and not model_args.use_waypoint_head:
        raise ValueError(
            "`--use_omninav_waypoint True` requires `--use_waypoint_head True`."
        )
    ...
    if model_args.use_waypoint_head:
        waypoint_config = WaypointConfig.from_dict(config.to_dict())
        waypoint_config.num_waypoints = model_args.num_waypoints
        waypoint_config.waypoint_loss_weight = model_args.waypoint_loss_weight
        waypoint_config.angle_loss_weight = model_args.angle_loss_weight
        waypoint_config.arrive_loss_weight = model_args.arrive_loss_weight
        waypoint_config.use_lm_loss = model_args.use_lm_loss_for_waypoint
    ...
    if data_args.use_omninav_waypoint:
        if data_args.omninav_data_base_path is None or data_args.omninav_video_base_path is None:
            raise ValueError(
                "`--omninav_data_base_path` and `--omninav_video_base_path` are required when "
